...
template<>
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
auto Lerp(const GOOM::Point2dFlt& val1,
          const GOOM::Point2dFlt& val2,
          const float t) noexcept -> GOOM::Point2dFlt
{
...
public:
  ...
  Lerper(uint32_t numSteps, const T& value1, const T& value2) noexcept;
...
template <Lerpable T>
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
Lerper<T>::Lerper(const uint32_t numSteps, const T& value1, const T& value2) noexcept
  : m_t{TValue::NumStepsProperties{.stepType=TValue::StepType::SINGLE_CYCLE, .numSteps = numSteps}},
    m_value1{value1},
    m_value2{value2}
{
}
